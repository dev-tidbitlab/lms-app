        ...
        return (
            <View style={{ margin: 10, backgroundColor: '#FFF', borderRadius: 10, overflow: 'hidden', width: 280 }}>
                ...
                <TouchableOpacity onPress={() => this.goToPortal(item.item)} style={{ padding: 5 }}>
                    <Text style={{ fontWeight: '600', color: '#373737', ...fonts.h7, paddingLeft: 5, paddingTop: 5, paddingBottom: 3, paddingRight: 5 }}>{item.item.courseName}</Text>
                    <Text style={{ fontWeight: '400', color: '#AAA', ...fonts.h8, paddingLeft: 5, paddingBottom: 3, paddingRight: 5 }} numberOfLines={2}>{item.item.description}</Text>
                    <View style={{ flexDirection: 'row', marginBottom: 15, marginTop: 5 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', paddingLeft: 5 }}>
                            {/* <Text style={{ fontWeight: '700', ...fonts.h6, paddingRight: 5 }}>{item.item.rating}</Text> */}
                            <View style={{ alignItems: 'flex-start' }}>
                                ...
                                <Text style={{ fontWeight: '500', color: "#AAA", ...fonts.h9, paddingTop:5 }}>Based on 50 review(s)</Text>
                            </View>
                        </View>
                        <View style={{ position: 'absolute', right: 5 }}>
                            {Number(item.item.price) != Number(item.item.sellPrice) ? <Text style={{ textDecorationLine: 'line-through', color: '#AAA', ...fonts.h7 }}>₹{item.item.price}</Text> : null}
                            <Text style={{...fonts.h7}}>₹{item.item.sellPrice}</Text>
                        </View>
                    </View>
                </TouchableOpacity>
            </View>
